...
# Yield Data
# -----------------------------------------
dt_yield <- read.csv('data/yield/dt_final.csv')


wheat_raw <- dt_yield %>% filter(fruchtart=='Winterweizen')
raps_raw <- dt_yield %>% filter(fruchtart=='Raps')

lapply(list(wheat_raw,raps_raw),head)

# Selecting required columns from yield data
wheat_sub <- dt_yield %>%
  dplyr::select(referenznummer, standort.laenge, standort.breite, value, year, fruchtart) %>%
...
# Checking wheat and rapeseed and assign to farm_yields for future use
...
#ggsave(plot=models_payouts_wheat, file="models_payouts_wheat.png", width = 16, height = 20, unit="cm")

...
# ID "2019028" farm from barley and "2018007", "2020031" have been omitted because they had values equal to 0
barley_raw <- subset(barley_raw, ID != "2019028")
rapeseed_raw <- rapeseed_raw[!(rapeseed_raw$ID %in% c("2018007", "2020031")), ]

# Checking both barley and rapeseed raw data 
...
barley_sub <- barley_mrg[c('ID','lon','lat','Yield','Year')]
rapeseed_sub <- rapeseed_mrg[c('ID','lon','lat','Yield','Year')]

# Transforming to wider form
barley <- reshape2::dcast(barley_sub, ID+lon+lat~Year,value.var = 'Yield') 
rapeseed <- reshape2::dcast(rapeseed_sub, ID+lon+lat~Year,value.var = 'Yield')

barley$ID <- NULL
rapeseed$ID <- NULL

lapply(list(barley,rapeseed),head)
farm_yields <- list(barley,rapeseed)

# Creating coordinate object for yield data
barley_coord <- barley[,1:2]
rapeseed_coord <- rapeseed[,1:2]
coordinates(barley_coord) <- c("lon", "lat")
coordinates(rapeseed_coord) <- c("lon", "lat")

# Setting proper CRS for the coordinate system
proj4string(barley_coord) <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
proj4string(rapeseed_coord) <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")

# Assign both crops' coordinate to list for future use
farm_coordinates <- list(barley_coord,rapeseed_coord)
...
# Second Part 2011-2015
start_panel <- as.Date("2011-01-01", format ="%Y-%m-%d")
end_panel   <- as.Date("2015-12-31", format="%Y-%m-%d")
...
# Specify the CRS to transform the object
...
for(i in 1:length(years)){
  ...
  file_nm <- paste0("data/tif/DOY_204-15_",years[i],'.tif')
...
barley_start <- do.call(cbind,list_dt)
names(barley_start) <- paste0('start_',2000:2015)
...
for(i in 1:length(years)){
  ...
  file_nm <- paste0("data/tif/DOY_204-21_",years[i],'.tif')
  DOY_tmp <- raster(file_nm)
  raster_crs <- proj4string(DOY_tmp)
  shape_raster_crs <- spTransform(shape_transformed, CRS(raster_crs))
  extracted_tmp <- raster::extract(DOY_tmp,shape_raster_crs,df = TRUE)[,2]
...
# Binding list_dt as data.frame
barley_end <- do.call(cbind,list_dt)
names(barley_end) <- paste0('end_',years)
...
PhenoFarm_barley_start <- barley_start
PhenoFarm_barley_end   <- barley_end
...
# List for each farm: farm_barley_exposure [[i]] [[t]] with i == farm, t == year
farm_barley_exposure <- list()

for (i in 1:nrow(farm_yields[[1]])){
  print(paste0('barley i: ',i))
  ...
  for (y in 1:(last_year - 2000 + 1)){
    start_risk <- as.Date(PhenoFarm_barley_start[i,y], formate = "%Y-%m-%d")
    # End the risk period before entrance into new growth phase (-1)
    end_risk   <- as.Date(PhenoFarm_barley_end [i,y], formate = "%Y-%m-%d")-1
  ...
  farm_barley_exposure[[i]] <- years_exposure 
  rm(years_exposure)
}
...
# Barley yields
barley_yield_melted           <- reshape2::melt(cbind(farm_yields[[1]][,-c(1:2)],id=1:nrow(farm_yields[[1]])),id="id")
colnames(barley_yield_melted) <- c("Farm","year","yield")
barley_yield_melted$year      <- as.numeric(as.character(barley_yield_melted$year))

# Temperature exposure barley
temp_barley_exposure1           <- rbindlist(lapply(farm_barley_exposure, rbindlist),idcol=T)
temp_barley_exposure2           <- reshape2::melt(temp_barley_exposure1, id=c(".id","date"))
colnames(temp_barley_exposure2) <- c("Farm","Date","Hour", "Temperature")
temp_barley_exposure2$year      <- year(temp_barley_exposure2$Date)
...
lin_trend_barley     <- rlm(yield ~ year , data = barley_yield_melted, method="M")
quadr_trend_barley  <- rlm(yield ~ year + I(year^2) , data=barley_yield_melted, method="M")

# Use Akaike information criterion (AIC) to identify more appropriate trend
AIC(lin_trend_barley)
AIC(quadr_trend_barley)

# AIC favors quadratic trend for farm-individual winter barley yields (lower value)
# Calculation of detrended yields using a quadratic trend
detr_barley_yields <- as.data.frame(matrix(NA, nrow=nrow(farm_yields[[1]]), ncol=length(seq(2000,last_year,1))))
colnames(detr_barley_yields) <- seq(2000,last_year,1)

for (i in 1:nrow(detr_barley_yields)){  
  for (t in 1:length(seq(2000,last_year,1))){
    cat('\014')
    cat(i)
    detr_barley_yields[i,t] <- ((coef(quadr_trend_barley)[1] +last_year * coef(quadr_trend_barley)[2] + (last_year^2) * coef(quadr_trend_barley)[3])) - ((coef(quadr_trend_barley)[1] + as.numeric(colnames(detr_barley_yields)[t]) * coef(quadr_trend_barley)[2] + (as.numeric(colnames(detr_barley_yields)[t]))^2 * coef(quadr_trend_barley)[3])) +  farm_yields[[1]][i,colnames(detr_barley_yields)[t]]
    
  }
}

# Utility of uninsured status
utility_barley_uninsured <- as.data.frame(matrix(NA, nrow=nrow(farm_yields[[1]]), ncol = length(seq(2000,last_year,1))))
for (i in 1:nrow(farm_yields[[1]])){
  for (t in 1:length(seq(2000,last_year,1))){
    utility_barley_uninsured[i,t] <- utility_function(alpha = alpha, revenue = detr_barley_yields[i,t])
  }
}

# Expected utility statistics
summary_EU_barley_uninsured <- as.data.frame(matrix(NA, nrow=nrow(farm_yields[[1]]),ncol=4))
colnames(summary_EU_barley_uninsured) <- c("EU", "Expected Revenue", "CE", "Risk Premium")

for (i in 1:nrow(farm_yields[[1]])){
  
  # Expected utility
  summary_EU_barley_uninsured [i,1] <- mean(as.numeric(utility_barley_uninsured[i,]), na.rm=T)
  
  # Expected Revenue
  summary_EU_barley_uninsured [i,2] <- mean(as.numeric(detr_barley_yields[i,]), na.rm=T)
  
  # Certainty equivalent
  summary_EU_barley_uninsured [i,3] <- inverse_utility_function(alpha = alpha, eu = as.numeric(summary_EU_barley_uninsured[i,1])) 
  
  # Risk Premium
  summary_EU_barley_uninsured [i,4] <- summary_EU_barley_uninsured [i,2] - summary_EU_barley_uninsured [i,3] 
}
...
# ==============================================================================================================
# 7. Winter barley: out-of-sample calibration and testing for three knots
# ==============================================================================================================
...
list_knot_locations_kn3_barley <- list()

for (i in 1:nrow(farm_yields[[1]])){
  # Leave-out data from farm i
  temp_sample <- temp_barley_exposure2[which(temp_barley_exposure2$Farm != i),]
  ...
  for (c in 1:ncol(knots_3_combi_good)){
    ...
    sub_barley_yield_melted <- barley_yield_melted[which(barley_yield_melted$Farm != i),]
    temp4 <- join(sub_barley_yield_melted,temp3, by=c("Farm", "year"))
    
    # Run regression
    temp_reg <- lm(yield ~ x + V2 + year + I(year^2) + as.factor(Farm)-1, data = temp4)
    RSS_3knots[c] <- sum(resid(temp_reg)^2)
    
    rm(temp_reg,temp4,sub_barley_yield_melted,temp3,temp2,temp1)
  ...
  list_knot_locations_kn3_barley[[i]] <-  temp_knot_df
  rm(temp_sample, temp_knot_df)
  print(i / nrow(farm_yields[[1]]))
}

#save.image('env_until_733_d3.RData')
...
for (i in 1:nrow(farm_yields[[1]])){
  ...
  # The RCS model
  # Get new time series and aggregate hourly values  
  temp1 <- as.matrix(rcspline.eval(temp_sample$Temperature,knots= list_knot_locations_kn3_barley[[i]][,1], inclx=T))
  temp2 <- cbind(temp_sample,temp1)
  ...
  rm(temp_sample, temp1, temp2, temp3,sub_barley_yield_melted,temp_cub_reg, temp_lin_reg,AIC_lin, AIC_cub)
}
...
for (i in 1:nrow(farm_yields[[1]])){
  ...
  for (m in 1:ncol(list_knot_locations_kn3_barley[[i]])){
    print(paste0('m:',m))
    # Get new time series for RCS   
    temp1 <- as.matrix(rcspline.eval(temp_barley_exposure2$Temperature,knots= list_knot_locations_kn3_barley[[i]][,m], inclx=T))
    temp2 <- cbind(temp_barley_exposure2, temp1)
    ...
    temp_hourly[[m]]     <- temp2
    temp_aggregated[[m]] <- join(barley_yield_melted,temp3, by=c("Farm", "year"))
    
    rm(temp1,temp2,temp3)
  }
  list_barley_matrix_modelkn3_aggregated[[i]] <- temp_aggregated
  list_barley_matrix_modelkn3_hourly[[i]] <- temp_hourly
...
list_farm_barley_panelmodelkn3_modeloutputs <- list()

for (i in 1:nrow(farm_yields[[1]])){
  print(i)
  # Contains the m models of farm i.
  list_farm_barley_panelmodelkn3_models.of.farm <- list() 
  for (m in 1:ncol(list_knot_locations_kn3_barley[[i]])){
    print(paste0('m ',m))
    # Aggregated data without farm i (leave-out-farm i for out-of-sample training)
    temp1 <- subset(list_barley_matrix_modelkn3_aggregated[[i]][[m]], list_barley_matrix_modelkn3_aggregated[[i]][[m]] [,"Farm"] != i)
    
    # Run panel regression
    temp_reg <- lm(yield ~ x + V2 + year + I(year^2) + as.factor(Farm)-1, data = temp1)
    list_farm_barley_panelmodelkn3_models.of.farm[[m]] <- temp_reg
    
    rm(temp1,temp_reg)
    
  }
  
  list_farm_barley_panelmodelkn3_modeloutputs [[i]] <- list_farm_barley_panelmodelkn3_models.of.farm   
  rm(list_farm_barley_panelmodelkn3_models.of.farm)
}


# Calculation of hourly effects (marginal temperature effect estimated at farm i)
list_farm_barley_hourly_effect_kn3 <- list() # [[farm]] [[model]][]
...
for (i in 1:nrow(farm_yields[[1]])){
  ...
  for (m in 1:ncol(list_knot_locations_kn3_barley[[i]])){
    ...
    print(paste0('m ',m))
    # Subset of farm i
    temp1 <- which(list_barley_matrix_modelkn3_hourly[[i]][[m]][,"Farm"] == i) 
    temp2 <- list_barley_matrix_modelkn3_hourly[[i]][[m]][temp1,]  
    
    # Calculate temperature effect
    temp2$effect <- as.matrix(temp2[,6:7]) %*% coef(list_farm_barley_panelmodelkn3_modeloutputs [[i]][[m]])[1:2]
    
    temp_list[[m]] <- temp2  
    rm(temp1,temp2)  
  }
  list_farm_barley_hourly_effect_kn3[[i]] <- temp_list
  rm(temp_list)
}
...
list_farm_barley_payouts_kn3 <- list() 
for (i in 1:nrow(farm_yields[[1]])){
  ...
  for (m in 1:ncol(list_knot_locations_kn3_barley[[i]])){
    ...
    temp_model <- list_farm_barley_hourly_effect_kn3 [[i]] [[m]]
  ...
  list_farm_barley_payouts_kn3[[i]] <- temp_list
  rm(temp_list)
} 

# Calculate the premium
# Structure: [farm i, model m, strike level]
array_premium_barley_kn3 <- array(dim=c(nrow(farm_yields[[1]]), ncol(list_knot_locations_kn3_barley[[i]]),length(strike_temperature)))
dimnames(array_premium_barley_kn3)[[3]] <- strike_temperature

for (i in 1:nrow(farm_yields[[1]])){
  for (m in 1:ncol(list_knot_locations_kn3_barley[[i]])){
    print(paste0('i:',i,'m:',m))
    for (strike in 1:length(strike_temperature)){
      temp1 <- which(!is.na(detr_barley_yields[i,]))
      array_premium_barley_kn3[i,m,strike] <- mean(as.numeric(list_farm_barley_payouts_kn3[[i]][[m]][strike, temp1]), na.rm=T)
      rm(temp1)
    }
  }
}
...
list_farm_barley_wealth_modelskn3_strike <- list()

for (strike in 1:length(strike_temperature)){
  
  array_farm_barley_wealth_modelskn3 <- array(dim=c(ncol(list_knot_locations_kn3_barley[[1]]), nrow(farm_yields[[1]]), length(seq(2000,last_year,1))))
  dimnames(array_farm_barley_wealth_modelskn3) [[3]] <-seq(2000,last_year,1) 
  
  for (m in 1:ncol(list_knot_locations_kn3_barley[[1]])){
    for (i in 1:nrow(farm_yields[[1]])){
      print(paste0('strike:',strike,' m:',m,' i:',i))
      for (t in 1: length(seq(2000,last_year,1))){
        
        array_farm_barley_wealth_modelskn3 [m,i,t] <- as.numeric(detr_barley_yields[i,t]) + as.numeric(list_farm_barley_payouts_kn3 [[i]][[m]] [strike,t]) - as.numeric(array_premium_barley_kn3[i,m,strike]) 
        
      }
    }
  }
  
  list_farm_barley_wealth_modelskn3_strike [[strike]] <- array_farm_barley_wealth_modelskn3 
  rm(array_farm_barley_wealth_modelskn3)
}

# Utility of being insured with specification m.
# List has structure: [[strike]][mi,i,t]
list_farm_barley_wealth_modelkn3_strike_utility <- list()

for (strike in 1:length(strike_temperature)){
  
  temp_array <- array(dim=c(ncol(list_knot_locations_kn3_barley[[i]]), nrow(farm_yields[[1]]), length(seq(2000,last_year,1))))
  ...
  for (m in 1:ncol(list_knot_locations_kn3_barley[[i]])){
    for (i in 1:nrow(farm_yields[[1]])){
      for (t in 1: length(seq(2000,last_year,1))){
        ...
        temp_array [m,i,t] <- utility_function(alpha = alpha, revenue = list_farm_barley_wealth_modelskn3_strike [[strike]][m,i,t])
        
      }
    }
  }
  
  list_farm_barley_wealth_modelkn3_strike_utility  [[strike]] <- temp_array 
  rm(temp_array)
}

# Expected utility statistics of being insured
# Structure of list: 
summary_EU_barley_insured_kn3 <- list() # [[strike]] [m,i,c(EU,E,CE,RP)]

for (strike in 1:length(strike_temperature)){
  temp_array <- array(dim=c(ncol(list_knot_locations_kn3_barley[[1]]), nrow(farm_yields[[1]]), 4))
  ...
  for (m in 1:ncol(list_knot_locations_kn3_barley[[1]])){
    for (i in 1:nrow(farm_yields[[1]])){
      ...
      # Expected utility
      temp_array  [m,i,1] <- mean(as.numeric(list_farm_barley_wealth_modelkn3_strike_utility[[strike]][m,i,]), na.rm=T)
      
      # Expected Revenue
      temp_array  [m,i,2] <- mean(as.numeric(list_farm_barley_wealth_modelskn3_strike[[strike]][m,i,]), na.rm=T)
  ...
  summary_EU_barley_insured_kn3 [[strike]] <- temp_array
  rm(temp_array)
}
...
average_rel_change_barley_knot3_uninsured <- as.data.frame(matrix(nrow=ncol(list_knot_locations_kn3_barley[[1]]), ncol=length(strike_temperature)))
colnames(average_rel_change_barley_knot3_uninsured) <- strike_temperature

for (m in 1:ncol(list_knot_locations_kn3_barley[[1]])){
  for (strike in 1:length(strike_temperature)){
    
    average_rel_change_barley_knot3_uninsured [m,strike] <- mean((summary_EU_barley_insured_kn3[[strike]][m,,4] - summary_EU_barley_uninsured[,4]) / summary_EU_barley_uninsured[,4])
    
  }
}

average_rel_change_barley_knot3_uninsured_test <- as.data.frame(matrix(nrow=ncol(list_knot_locations_kn3_barley[[1]]), ncol=length(strike_temperature)))
colnames(average_rel_change_barley_knot3_uninsured_test) <- strike_temperature

for (m in 1:ncol(list_knot_locations_kn3_barley[[1]])){
  for (strike in 1:length(strike_temperature)){
    
    average_rel_change_barley_knot3_uninsured_test [m,strike] <- wilcox.test(summary_EU_barley_insured_kn3[[strike]][m,,4], summary_EU_barley_uninsured[,4], paired=T, alternative = "l")$p.value
...
# ==============================================================================================================
# 9. Winter barley: out-of-sample calibration and testing for five knots
# ==============================================================================================================
...
knot_locations_kn5_barley            <- as.data.frame(matrix(NA, nrow=5, ncol=1))
row.names(knot_locations_kn5_barley) <- c("knot1", "knot2", "knot3","knot4","knot5")
colnames(knot_locations_kn5_barley)  <- c("Schlenker") 

# Equally spaced across temperature support
knot_locations_kn5_barley [,1] <- c(5,10,15,20,25)


'# quantiles (see also robustness checks)
knot_locations_kn5_barley [,2] <- c(quantile(temp_barley_exposure2$Temperature,0.05, type=1),
                                    quantile(temp_barley_exposure2$Temperature,0.275, type=1),
                                    quantile(temp_barley_exposure2$Temperature,0.5, type=1),
                                    quantile(temp_barley_exposure2$Temperature,0.725, type=1),
                                    quantile(temp_barley_exposure2$Temperature,0.95, type=1))'
...
list_barley_matrix_modelkn5_aggregated <- list() # [[m]]

# New time series of hourly temperature exposures: [[m]] == new time series for knot specification m
list_barley_matrix_modelkn5_hourly <- list() # [[m]]

for (m in 1:ncol(knot_locations_kn5_barley)){
  print('m: ',m)
  # Get new time series for RCS   
  temp1 <- as.matrix(rcspline.eval(temp_barley_exposure2$Temperature,knots= knot_locations_kn5_barley[,m], inclx=T))
  temp2 <- cbind(temp_barley_exposure2, temp1)
  ...
  list_barley_matrix_modelkn5_hourly[[m]]     <- temp2
  list_barley_matrix_modelkn5_aggregated[[m]] <- join(barley_yield_melted,temp3, by=c("Farm", "year"))
...
# List with model outputs. [[i]] == farm i; [[m]] == knot specification m
list_farm_barley_panelmodelkn5_modeloutputs <- list()

for (i in 1:nrow(farm_yields[[1]])){
  
  # Contains the m models of farm i.
  list_farm_barley_panelmodelkn5_models.of.farm <- list() 
  for (m in 1:ncol(knot_locations_kn5_barley)){
    print(paste0('m: ',m,'i: ',i))    
    # Aggregated data without farm i (leave-out-farm i for out-of-sample training)
    temp1 <- subset(list_barley_matrix_modelkn5_aggregated[[m]], list_barley_matrix_modelkn5_aggregated[[m]] [,"Farm"] != i)
    
    # Run panel regression
    temp_reg <- lm(yield ~ x + V2+V3+V4 + year + I(year^2) + as.factor(Farm)-1, data = temp1)
    list_farm_barley_panelmodelkn5_models.of.farm[[m]] <- temp_reg
    
    rm(temp1,temp_reg)
    
  }
  
  list_farm_barley_panelmodelkn5_modeloutputs [[i]] <- list_farm_barley_panelmodelkn5_models.of.farm   
  rm(list_farm_barley_panelmodelkn5_models.of.farm)
}


# Calculation of hourly effects (marginal temperature effect)
list_farm_barley_hourly_effect_kn5 <- list() # [[farm]] [[model]][]
...
for (i in 1:nrow(farm_yields[[1]])){
  ...
  for (m in 1:ncol(knot_locations_kn5_barley)){
    print(paste0('i: ',i,' m: ',m))
    # Subset of farm i
    temp1 <- which(list_barley_matrix_modelkn5_hourly[[m]][,"Farm"] == i) 
    temp2 <- list_barley_matrix_modelkn5_hourly[[m]][temp1,]  
    
    # Calculate temperature effect
    temp2$effect <- as.matrix(temp2[,6:9]) %*% coef(list_farm_barley_panelmodelkn5_modeloutputs [[i]][[m]])[1:4]
    
    temp_list[[m]] <- temp2  
    rm(temp1,temp2)  
  }
  list_farm_barley_hourly_effect_kn5[[i]] <- temp_list
  rm(temp_list)
}
...
# Historical payouts with following structure: [[i]] == of farm i, [[m]] with model specification m, 
# [[m]] contains a data.frame with [strike level, year]
list_farm_barley_payouts_kn5 <- list() 
for (i in 1:nrow(farm_yields[[1]])){
  ...
  for (m in 1:ncol(knot_locations_kn5_barley)){
    print(paste0('i: ',i,'m: ',m))      
    temp_model <- list_farm_barley_hourly_effect_kn5 [[i]] [[m]]
  ...
  list_farm_barley_payouts_kn5[[i]] <- temp_list
  rm(temp_list)
} 

# Calculate the premium
# Structure: [farm i, model m, strike level]
array_premium_barley_kn5 <- array(dim=c(nrow(farm_yields[[1]]), ncol(knot_locations_kn5_barley),length(strike_temperature)))
dimnames(array_premium_barley_kn5)[[3]] <- strike_temperature
...
for (i in 1:nrow(farm_yields[[1]])){
  for (m in 1:ncol(knot_locations_kn5_barley)){
    for (strike in 1:length(strike_temperature)){
      print(paste0(i,'  ',m))
      temp1 <- which(!is.na(detr_barley_yields[i,]))
      array_premium_barley_kn5[i,m,strike] <- mean(as.numeric(list_farm_barley_payouts_kn5[[i]][[m]][strike, temp1]), na.rm=T)
      rm(temp1)
    }  
  }
}
...
# Calculation of revenue [[strike]][m,i,t]
list_farm_barley_wealth_modelskn5_strike <- list()

for (strike in 1:length(strike_temperature)){
  print(strike)  
  array_farm_barley_wealth_modelskn5 <- array(dim=c(length(knot_locations_kn5_barley), nrow(farm_yields[[1]]), length(seq(2000,last_year,1))))
  dimnames(array_farm_barley_wealth_modelskn5) [[3]] <-seq(2000,last_year,1) 
  
  for (m in 1:ncol(knot_locations_kn5_barley)){
    for (i in 1:nrow(farm_yields[[1]])){
      for (t in 1: length(seq(2000,last_year,1))){
        
        array_farm_barley_wealth_modelskn5 [m,i,t] <- as.numeric(detr_barley_yields[i,t]) + as.numeric(list_farm_barley_payouts_kn5 [[i]][[m]] [strike,t]) - as.numeric(array_premium_barley_kn5[i,m,strike]) 
        
      }
    }
  }
  
  list_farm_barley_wealth_modelskn5_strike [[strike]] <- array_farm_barley_wealth_modelskn5 
  rm(array_farm_barley_wealth_modelskn5)
}

# Utility of being insured with specification m.
# List has structure: [[strike]][m,i,t]
list_farm_barley_wealth_modelkn5_strike_utility <- list()

for (strike in 1:length(strike_temperature)){
  print(strike)  
  temp_array <- array(dim=c(length(knot_locations_kn5_barley), nrow(farm_yields[[1]]), length(seq(2000,last_year,1))))
  dimnames(temp_array) [[3]] <-seq(2000,last_year,1) 
  ...
  for (m in 1:ncol(knot_locations_kn5_barley)){
    for (i in 1:nrow(farm_yields[[1]])){
      for (t in 1: length(seq(2000,last_year,1))){
        ...
        temp_array [m,i,t] <- utility_function(alpha = alpha, revenue = list_farm_barley_wealth_modelskn5_strike [[strike]][m,i,t])
        
      }
    }
  }
  
  list_farm_barley_wealth_modelkn5_strike_utility  [[strike]] <- temp_array 
  rm(temp_array)
}

# Expected utility statitsics of being insured
# Structure of list: 
summary_EU_barley_insured_kn5 <- list() # [[strike]] [m,i,c(EU,E,CE,RP)]

for (strike in 1:length(strike_temperature)){
  temp_array <- array(dim=c(length(knot_locations_kn5_barley), nrow(farm_yields[[1]]), 4))
  dimnames(temp_array) [[3]] <- c("EU", "Expected Revenue", "CE", "RP")
  ...
  for (m in 1:ncol(knot_locations_kn5_barley)){
    for (i in 1:nrow(farm_yields[[1]])){
      ...
      # Expected utility
      temp_array  [m,i,1] <- mean(as.numeric(list_farm_barley_wealth_modelkn5_strike_utility[[strike]][m,i,]), na.rm=T)
      
      # Expected Revenue
      temp_array  [m,i,2] <- mean(as.numeric(list_farm_barley_wealth_modelskn5_strike[[strike]][m,i,]), na.rm=T)
  ...
  summary_EU_barley_insured_kn5 [[strike]] <- temp_array
  rm(temp_array)
}
...
average_rel_change_barley_knot5_uninsured <- as.data.frame(matrix(nrow=ncol(knot_locations_kn5_barley), ncol=length(strike_temperature)))
colnames(average_rel_change_barley_knot5_uninsured) <- strike_temperature

for (m in 1:ncol(knot_locations_kn5_barley)){
  for (strike in 1:length(strike_temperature)){
    
    average_rel_change_barley_knot5_uninsured [m,strike] <- mean((summary_EU_barley_insured_kn5[[strike]][m,,4] - summary_EU_barley_uninsured[,4]) / summary_EU_barley_uninsured[,4])
    
  }
}

average_rel_change_barley_knot5_uninsured_test <- as.data.frame(matrix(nrow=ncol(knot_locations_kn5_barley), ncol=length(strike_temperature)))
colnames(average_rel_change_barley_knot5_uninsured_test) <- strike_temperature

for (m in 1:ncol(knot_locations_kn5_barley)){
  for (strike in 1:length(strike_temperature)){
    
    average_rel_change_barley_knot5_uninsured_test [m,strike] <-wilcox.test(summary_EU_barley_insured_kn5[[strike]][m,,4], summary_EU_barley_uninsured[,4], paired=T, alternative="l")$p.value
...
# Content:
# 1) Figure 2: hourly temperature effects and payouts for winter barley
...
sub_temperature_exposure_barley <- temp_barley_exposure2[which(temp_barley_exposure2$Farm != farm_ID),]

temperature_barley <- ggplot(sub_temperature_exposure_barley, aes(x=Temperature)) + 
...
temperatures_ts_M1 <- rcspline.eval(sub_temperature_exposure_barley$Temperature,knots= list_knot_locations_kn3_barley[[farm_ID]][,1], inclx=T)  

# Add new ts to dates
temp1 <- cbind(sub_temperature_exposure_barley, temperatures_ts_M1)

# Aggregation to yearly values
final_barley_temperature_aggregated_M1 <-temp1 %>%
  group_by(Farm,year)%>%
  summarise_at(vars(x,V2),sum)
rm(temp1)

# Combine yearly values with yields
temp1 <- barley_yield_melted[which(barley_yield_melted$Farm != farm_ID),]
data_barley_final_M1 <- join(temp1, final_barley_temperature_aggregated_M1, by=c("Farm","year"))
rm(temperatures_ts_M1,temp1,final_barley_temperature_aggregated_M1)

# New TS for temperature support of interest
marginal_temperatures_M1 <- rcspline.eval(seq(-10,39,0.225),knots=list_knot_locations_kn3_barley[[farm_ID]][,1], inclx=T)

# Regression output with reliable R-squared
temp_reg1 <- feols(yield ~ x + V2 + year + I(year^2) | Farm, data=data_barley_final_M1)
summary(temp_reg1)
...
for (l in 1: 1000){
  # Sample some years
  data_temp <- data_barley_final_M1[which(data_barley_final_M1$year %in% sample(unique(data_barley_final_M1$year),replace=T)),]
...
model_1 <- ggplot()+ ggtitle("Hourly temperature effects: Model 1") + 
  ...
  geom_vline(xintercept=list_knot_locations_kn3_barley[[farm_ID]][,1], linetype="dotted", colour="gray")+
...
payout_barley_model_1 <- ggplot(temp) +
...
# Temperature time series
temperatures_ts_M2 <- rcspline.eval(sub_temperature_exposure_barley$Temperature,knots= list_knot_locations_kn3_barley[[farm_ID]][,2], inclx=T)  

# Add new ts to dates
temp1 <- cbind(sub_temperature_exposure_barley, temperatures_ts_M2)

# Aggregation to yearly values
final_barley_temperature_aggregated_M2 <-temp1 %>%
  group_by(Farm,year)%>%
  summarise_at(vars(x,V2),sum)
rm(temp1)
...
# Combine yearly values with yields
temp1 <- barley_yield_melted[which(barley_yield_melted$Farm != farm_ID),]
data_barley_final_M2 <- join(temp1, final_barley_temperature_aggregated_M2, by=c("Farm","year"))
rm(temperatures_ts_M2,temp1,final_barley_temperature_aggregated_M2)

# New TS for temperature support of interest
marginal_temperatures_M2 <- rcspline.eval(seq(-10,39,0.225),knots=list_knot_locations_kn3_barley[[farm_ID]][,2], inclx=T)

# Regression output with reliable R-squared
temp_reg1 <- feols(yield ~ x + V2 + year + I(year^2) | Farm, data=data_barley_final_M2)
summary(temp_reg1)
rm(temp_reg1)
...
for (l in 1: 1000){
  # Sample some years
  data_temp <- data_barley_final_M2[which(data_barley_final_M2$year %in% sample(unique(data_barley_final_M2$year),replace=T)),]
...
model_2 <- ggplot()+ ggtitle("Hourly temperature effects: Model 2") + 
  ...
  scale_y_continuous(breaks = c(-0.5,-0.4,-0.3,-0.2,-0.1,0,0.1), limits = c(-0.4,0.1))+
  geom_hline(yintercept=c(seq(-0.5,0.1,by=0.1)), colour="gray")+
  geom_vline(xintercept=list_knot_locations_kn3_barley[[farm_ID]][,2], linetype="dotted", colour="gray")+
...
payout_barley_model_2 <- ggplot(temp) +
...
# Temperature time series
temperatures_ts_M3 <- rcspline.eval(sub_temperature_exposure_barley$Temperature,knots= list_knot_locations_kn3_barley[[farm_ID]][,3], inclx=T)  

# Add new ts to dates
temp1 <- cbind(sub_temperature_exposure_barley, temperatures_ts_M3)

# Aggregation to yearly values
final_barley_temperature_aggregated_M3 <-temp1 %>%
  group_by(Farm,year)%>%
  summarise_at(vars(x,V2),sum)
rm(temp1)
...
# Combine yearly values with yields
temp1 <- barley_yield_melted[which(barley_yield_melted$Farm != farm_ID),]
data_barley_final_M3 <- join(temp1, final_barley_temperature_aggregated_M3, by=c("Farm","year"))
rm(temperatures_ts_M3,temp1,final_barley_temperature_aggregated_M3)

# New TS for temperature support of interest
marginal_temperatures_M3 <- rcspline.eval(seq(-10,39,0.225),knots=list_knot_locations_kn3_barley[[farm_ID]][,3], inclx=T)

temp_reg1 <- feols(yield ~ x + V2 + year + I(year^2) | Farm, data=data_barley_final_M3)
summary(temp_reg1)
rm(temp_reg1)
...
for (l in 1: 1000){
  # Sample some years
  data_temp <- data_barley_final_M3[which(data_barley_final_M3$year %in% sample(unique(data_barley_final_M3$year),replace=T)),]
...
model_3 <- ggplot()+ ggtitle("Hourly temperature effects: Model 3") + 
  ...
  scale_y_continuous(breaks = c(-0.5,-0.4,-0.3,-0.2,-0.1,0,0.1), limits = c(-0.4,0.1))+
  geom_hline(yintercept=c(seq(-0.5,0.1,by=0.1)), colour="gray")+
  geom_vline(xintercept=list_knot_locations_kn3_barley[[farm_ID]][,3], linetype="dotted", colour="gray")+
...
payout_barley_model_3 <- ggplot(temp) +
...
# Temperature time series
temperatures_ts_M4 <- rcspline.eval(sub_temperature_exposure_barley$Temperature,knots= c(5,10,15,20,25), inclx=T)  

# Add new ts to dates
temp1 <- cbind(sub_temperature_exposure_barley, temperatures_ts_M4)

# Aggregation to yearly values
final_barley_temperature_aggregated_M4 <-temp1 %>%
  group_by(Farm,year)%>%
  summarise_at(vars(x,V2,V3,V4),sum)
rm(temp1)
...
# Combine yearly values with yields
temp1 <- barley_yield_melted[which(barley_yield_melted$Farm != farm_ID),]
data_barley_final_M4 <- join(temp1, final_barley_temperature_aggregated_M4, by=c("Farm","year"))
rm(temperatures_ts_M4,temp1,final_barley_temperature_aggregated_M4)

# New TS for temperature support of interest
marginal_temperatures_M4 <- rcspline.eval(seq(-10,39,0.225),knots=knot_locations_kn5_barley[,1], inclx=T)

temp_reg1 <- feols(yield ~ x + V2 +V3 +V4 + year + I(year^2) | Farm, data=data_barley_final_M4)
summary(temp_reg1)
rm(temp_reg1)
...
for (l in 1: 1000){
  # Sample some years
  data_temp <- data_barley_final_M4[which(data_barley_final_M4$year %in% sample(unique(data_barley_final_M4$year),replace=T)),]
...
model_4 <- ggplot()+ ggtitle("Hourly temperature effects: Model 4") + 
  ...
  scale_y_continuous(breaks = c(-0.5,-0.4,-0.3,-0.2,-0.1,0,0.1), limits = c(-0.4,0.1))+
  geom_hline(yintercept=c(seq(-0.5,0.1,by=0.1)), colour="gray")+
  geom_vline(xintercept=knot_locations_kn5_barley[,1], linetype="dotted", colour="gray")+
...
payout_barley_model_4 <- ggplot(temp) +
...
models_payouts_barley <- ggarrange(model_1,payout_barley_model_1,model_2,payout_barley_model_2,model_3,payout_barley_model_3,model_4,payout_barley_model_4,temperature_barley, ncol=2, byrow=T, heights = c(1,1,1,1,0.5))
#ggsave(plot=models_payouts_barley, file="models_payouts_barley.png", width = 16, height = 20, unit="cm")
rm(model_1,payout_barley_model_1,model_2,payout_barley_model_2,model_3,payout_barley_model_3,model_4,payout_barley_model_4,temperature_barley)
...
rel_RP_barley_models <- array(dim=c(3,nrow(farm_yields[[1]]), ncol=4))
dimnames(rel_RP_barley_models) [[1]] <- c("15", "20", "25")
dimnames(rel_RP_barley_models) [[3]] <- c("Model 1", "Model 2", "Model 3", "Model 4")

for (l in 1:length(strike_temperature_levels)){
  for (i in 1:nrow(farm_yields[[1]])){
    
    # Model 1: Best fit
    rel_RP_barley_models[l,i,1] <- (-100)*(summary_EU_barley_insured_kn3[[strike_temperature_levels[l]]] [1,i,4] - summary_EU_barley_uninsured[i,4]) / summary_EU_barley_uninsured[i,4]
    
    # Model 2: Equal spaces
    rel_RP_barley_models[l,i,2] <- (-100)*(summary_EU_barley_insured_kn3[[strike_temperature_levels[l]]] [2,i,4] - summary_EU_barley_uninsured[i,4]) / summary_EU_barley_uninsured[i,4]
    
    # Model 3: Quantiles
    rel_RP_barley_models[l,i,3] <- (-100)*(summary_EU_barley_insured_kn3[[strike_temperature_levels[l]]] [3,i,4] - summary_EU_barley_uninsured[i,4]) / summary_EU_barley_uninsured[i,4]
    
    # Model 4: Schlenker (5 space)
    rel_RP_barley_models[l,i,4] <- (-100)*(summary_EU_barley_insured_kn5[[strike_temperature_levels[l]]] [1,i,4] - summary_EU_barley_uninsured[i,4]) / summary_EU_barley_uninsured[i,4]
  }
}

# Plot for strike level 20
# rel_RP_barley_models[2,,] contains RP changes for strike level temperature 20

# data.frame for Asteriks
label.df <- data.frame(Var2=c("Model 1","Model 2","Model 3","Model 4"), Value = c(45,45,45,45))

rel_RP_barley_models_melted <-reshape2::melt(as.matrix(rel_RP_barley_models[2,,]))

barley_20 <- ggplot(rel_RP_barley_models_melted, aes(x=Var2, y=value)) + ggtitle("a) winter barley \n strike level: 20°C") +
...
rel_RP_barley_models_melted_25 <-reshape2::melt(as.matrix(rel_RP_barley_models[3,,]))

barley_25 <- ggplot(rel_RP_barley_models_melted_25, aes(x=Var2, y=value)) + ggtitle("b) winter barley \n strike level: 25°C") +
  geom_boxplot(notch = F, fill="gray", outlier.size = 0.1, coef=0) + 
  theme(panel.grid.major = element_blank(),
        plot.title = element_text(family="Times New Roman", size=12, colour = "black", hjust=0.5),
        axis.title = element_text(family="Times New Roman", size=10, colour = "black"),
...
plot_risk <- ggarrange(barley_20, barley_25,rapeseed_15,rapeseed_20, nrow=2, ncol=2)
